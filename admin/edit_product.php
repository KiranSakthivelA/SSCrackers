    ...
    <div class="container">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <div class="file-upload-wrapper" style="height: <?= $product['image_url'] ? '200px' : '150px' ?>;">
                    <input type="file" name="image" id="imageInput" accept="image/*">
                    <div class="file-upload-content" id="uploadContent" style="<?= $product['image_url'] ? 'display: none;' : '' ?>">
                        <i class="fas fa-cloud-upload-alt"></i><br>
                        <span>Drag & Drop to Replace Image</span>
                    </div>
                    <?php if($product['image_url']): ?>
                        <img id="imagePreview" src="<?= strpos($_SERVER['HTTP_HOST'], 'localhost') !== false ? '../' : 'https://sscrackers.in/' ?><?= htmlspecialchars($product['image_url']) ?>" alt="Preview" style="display: block;">
                    <?php else: ?>
                ...
                <?php if($product['image_url']): ?>
                    <label class="remove-img">
                        ...
                        <i class="fas fa-trash-alt"></i> Delete current image entirely
                    </label>
                <?php endif; ?>
            </div>
            <div class="btn-group">
    ...
    <script>
        ...
        imageInput.addEventListener('change', function() {
            ...
            if (file) {
                ...
                // If they cancel file selection, revert to previous state
                <?php if($product['image_url']): ?>
                    imagePreview.src = "<?= strpos($_SERVER['HTTP_HOST'], 'localhost') !== false ? '../' : 'https://sscrackers.in/' ?><?= htmlspecialchars($product['image_url']) ?>";
                    imagePreview.style.display = 'block';
                    uploadContent.style.display = 'none';
                <?php else: ?>
